...
function Projects() {
    ...
    for(var i =0; i<data.length;i++){
        ...
        if(i<3){
        projectsStart.push(<ProjectCard 
            title={data[i].title}
    ...
    <div className='projects-container'>
        ...
        <button className='nice-button' style={{fontWeight:"bold", fontSize:"20px", 
        visibility: otherProjects ? "hidden": "visible"}}
        onClick={
            ()=>setOtherProjects(true)
        }
        > See more projects <ArrowDropDown /></button>
        </div>
    );
}
